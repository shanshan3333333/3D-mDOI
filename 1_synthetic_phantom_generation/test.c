...
int main(int argc, const char * argv[]) {
    ...
    int     n_loc;           //number of location of monte carlo
	char 	*v=NULL;
	double  *PDF=NULL;
	char    *PDF_1photon=NULL;
	double  *n_count=NULL;  //ectuall number of the photon for each location
	int 	type;
	int     NNN;            //denote NN*n_loc
    n_loc=Nx/2;    //symmetric location
	NN = Nx*Ny*Nz;
    NNN= n_loc*NN;

	v  = ( char *)malloc(NN*sizeof(char));  /* tissue structure */
	PDF= (double *)malloc(NNN*sizeof(double));
	PDF_1photon= (char *)malloc(NN*sizeof(char));
    n_count= (double *)malloc(n_loc*sizeof(double));
    ...
    printf("INITIALIZATIONS FOR PDF finish\n");
	// read binary file
    strcpy(filename,myname);
	...
	do {
		...
		if (i_photon==1000) {    
			...
			printf("x-halfwidth axial count of number of exiting photon in 1000 trails:\n");
			for (int i=0; i<n_loc;i++)
			{
				printf("%0.0f\t",n_count[i]);
			}
			printf("\n");
		}
		...
		if (ix<n_loc && iy==Ny/2 && iz<=0) 
		{

		    for (int i=0; i<NN;i++)
		    {
		        PDF[ix*NN+i] += PDF_1photon[i];
			}
	...
	printf("total collect %5.3f photons\n",n_count);
	
    /**** SAVE
     Convert data and save
     *****/

    // normalize the PDF result
    ...
    strcpy(filename,myname);
    strcat(filename,"_PDF.bin");
    printf("saving %s\n",filename);
    fid = fopen(filename, "wb");   /* 3D voxel output */
    fwrite(PDF, sizeof(double), NNN, fid);
    fclose(fid);
    
    /* save n_count */
    strcpy(filename,myname);
    strcat(filename,"_COUNT.bin");
    printf("saving %s\n",filename);
    fid = fopen(filename, "wb");   /* 3D voxel output */
    fwrite(n_count, sizeof(double), n_loc, fid);
    fclose(fid);
