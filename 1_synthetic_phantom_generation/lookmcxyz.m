...
%% Look at Fluence Fxy at surface
Fxy = reshape(F(:,:,1),Ny,Nx)'; % in y,x plane through source

figure(2);clf
set(gcf,'color','w');
imagesc(y,x,log10(Fxy),[.5 2.8])
hold on
text(max(y)*1.2,min(x)-0.04*max(x),'log_{10}( \phi )','fontsize',fz)
colorbar
set(gca,'fontsize',sz)
xlabel('y [cm]')
ylabel('x [cm]')
title('Fluence at surface')
% title('Fluence at surface ','fontweight','normal','fontsize',fz)
colormap(makec2f)
axis equal image
